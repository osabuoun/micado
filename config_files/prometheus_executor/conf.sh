...
VM_over_loaded() {
  curl -X POST http://31.171.244.244:5000/infrastructures/$1/scaleup/$2
}

VM_under_loaded() {
  curl -X POST http://31.171.244.244:5000/infrastructures/$1/scaledown/$2
}

containerscale() {
  echo "scale $1 by $2"
  curl -X GET http://31.171.244.244:2375/v1.27/services/$1 > get.json
  # to pretty json
  ...
  if [ "$replicas" -gt "1" ] || [ $2 -eq "1" ]; then
    ...
    curl -g 'http://31.171.244.244:9090/api/v1/query?query=rate(node_cpu{mode="idle",group="worker_cluster"}[1m])' > available_nodes_forscale.json
    curl -g 'http://31.171.244.244:9090/api/v1/query?query=rate(node_cpu{mode="idle"}[1m])' | jq '. | length' > node_counter
    ...
    for i in $(seq 0 $num_nodes); do
      ...
      if [ $actual_free_cpu -gt $service_cpulimit ] || [ $2 -ne "1" ]; then
        echo "true scale"
        curl -X POST --header "Content-Type: application/json" http://31.171.244.244:2375/v1.27/services/$1/update?version=$index -d @output.json
        break
      else
        echo "no space"
      fi
...
main() {
  for i in $(seq 1 "$AMX_ALERT_LEN"); do
    ...
    if [ "${!alert}" = "overloaded" ] && [ "${!level}" = "VM" ] && [ "${!alertstatus}" = "firing" ]; then
      VM_over_loaded "${!infra}" "${!node}"
    fi

    if [ "${!alert}" = "underloaded" ] && [ "${!level}" = "VM" ] && [ "${!alertstatus}" = "firing" ]; then
